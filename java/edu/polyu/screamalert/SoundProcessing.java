...
public class SoundProcessing {
						...
						if (vad.isEventEnded() == true) {
							...
							if (score > Config.MIN_SCORE) {				// Send notification to Android
								...
								if (score > decisionTh) {
									...
									//firebase code
									/*SoundProcessingActivity.locationRef.child("lat").setValue(gps.getLatitude());
									SoundProcessingActivity.locationRef.child("lon").setValue(gps.getLongitude());*/
								}
							}
							try {
	...
	private static double getEventScore(ArrayList<double[]> aList) {
		...
		//firebase code
//		SoundProcessingActivity.scoreRef.setValue(score+1000);
	...
	private static void sendSMS() {
		...
		if (enableSMS == true && phoneNumber != null && phoneNumber.trim().length() > 0 &&
			...
			smsManager.sendTextMessage(phoneNumber, null, "Scream detected. Click her to view: https://maps.google.com/?q="+gps.getLatitude()+","+gps.getLongitude()+" Please try to contact the mobile user!", null, null);
		}
	}
